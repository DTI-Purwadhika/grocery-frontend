...
export const getPrimaryAddress = () => {
  ...
  useEffect(() => {
    const fetchPrimaryAddress = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/addresses/primary`, {
          method: "GET",
          headers: {
        ...
        setPrimaryAddressData({id: data.data.id, addressName: data.data.addressName, city: data.data.city.name})
      } catch (error) {
        console.error(error);
      }
    };
